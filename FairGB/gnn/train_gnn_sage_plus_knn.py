...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # separate weights for positive (targets=1) and negative (targets=0)
...
# Загружаем k‑NN граф (1 млн ребер) — он точно влезает в память
...
# Параметры (оптимизированы под 8 ГБ)
hidden_dim = 256
num_layers = 3
lr = 0.0003
batch_size = 512            # уменьшен для безопасности
num_neighbors = [8, 4]      # уменьшено, чтобы снизить расход памяти
num_epochs = 500
...
print(f"\nРезультаты SAGE+ с Focal Loss на k‑NN графе:")
