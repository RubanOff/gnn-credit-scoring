...
def load_german(dataset, sens_attr="Gender", predict_attr="GoodCustomer", path="dataset/german/", label_number=1000, test_idx=None):
    ...
    # Sensitive Attribute
    # Преобразуем столбец Gender в числовой: Male -> 1, Female -> 0
    idx_features_labels['Gender'] = idx_features_labels['Gender'].map({'Male': 1, 'Female': 0})
    ...
    features = sp.csr_matrix(idx_features_labels[header], dtype=np.float32)
    labels = idx_features_labels['GoodCustomer'].values.copy()
    labels[labels == -1] = 0
